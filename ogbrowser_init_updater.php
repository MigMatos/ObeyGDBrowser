...
class OGDBrowserUpdater
{
    ...
    {
        try {
            if ($this->db->query("SHOW TABLES LIKE 'tokenapp'")->rowCount() > 0) {
                $this->updateLogger("Table 'tokenapp' already created.", 90);
            }
            ...
            $this->db->exec("
                CREATE TABLE tokenapp (
                    ...
                    imageURLApp VARCHAR(255) NULL DEFAULT '',
                    accountID INT NOT NULL,
                    developer VARCHAR(100) NULL DEFAULT '',
                    registerDate INT NOT NULL,
                    expireDate INT NULL DEFAULT '-1',
    				alive INT NULL DEFAULT '0',
                    nameTokenApp VARCHAR(12) NULL DEFAULT '-',
                    token VARCHAR(255) NOT NULL,
                    PRIMARY KEY (id)
                ) ENGINE=InnoDB;
            ");
            $this->updateLogger("Table 'tokenapp' created.", $this->progressPercentage);
        } catch (PDOException $e) {
            $this->updateLogger("Error creating table: " . $e->getMessage(), $this->progressPercentage);
        }
    }
    ...
    {
        try {
            $this->db->exec("ALTER TABLE roles ADD ogdwCreateTokenApp INT NOT NULL DEFAULT '0';");
            $this->updateLogger("Column ogdwCreateTokenApp added.", $this->progressPercentage);
        } catch (PDOException $e) {
            $this->updateLogger("Error adding column ogdwCreateTokenApp: " . $e->getMessage(), $this->progressPercentage);
        }
    }
