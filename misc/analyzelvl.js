...
async function analyzeLevelFromServer(levelData) {
    ...
    const analysis = analyze_level(levelData, levelData.data);
    return analysis
...
async function loadJSON() {
    try {
        const jsonFiles = {
            objects: await fetch('../../misc/analysis/objects.json'),
            blocks: await fetch('../../misc/analysis/blocks.json'),
            properties: await fetch('../../misc/analysis/objectProperties.json'),
            init: await fetch('../../misc/analysis/initialProperties.json'),
            colorStuff: await fetch('../../misc/analysis/colorProperties.json')
        };

        for (const key in jsonFiles) {
            if (!jsonFiles[key].ok) {
                throw new Error(`Error loading JSON ${key}.json.`);
            }
            const data = await jsonFiles[key].json();
            // console.log(`Objeto JSON ${key}.json cargado:`, data);
            ...
            if (key === 'objects') {
                ...
                init = data;
            } else if (key == 'colorStuff') {
                colorStuff = data;
            }
        }
        jsonLoaded = true;
    } catch (error) {
        console.error('Error loading JSON function:', error);
        throw error; 
    }
}


function analyzeLevelData(datalvl) {
    if (!jsonLoaded) {
      console.log('Waiting for JSON to load...');
      const data = loadJSON().then(() => {

        return analyzeLevelFromServer(datalvl);
        //console.log(data);
      });
      return data;
