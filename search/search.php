...
function profileRedirect(url) {
	...
	openGdCustomFrame(queryProfile+"&gdframe");
}
	...
	res.forEach((x, y) => {
		...
		$('#searchBox').append(`<div class="searchresult" title="${clean(x.description)}">
			...
			<h2 class="pre smaller inline gdButton help ${hasAuthor ? "" : "green unregistered"}" title="Account ID: ${x.accountID}\nPlayer ID: ${x.playerID}"><!--
				-->${hasAuthor && !onePointNine ? `<a style="margin-right: 0.66vh" onclick=profileRedirect('${x.accountID}')>By ${x.author || "-"}</a>` : `<a ${userSearch ? "" : `onclick=profileRedirect('${x.playerID}','userID')`}>By ${x.author || "-"}</a>`}</h2><!--
				--><h2 class="inline" style="margin-left: 1.5%; transform:translateY(30%)"> ${x.copiedID == '0' ? "" : `<a target="_blank" onclick=levelRedirect('${x.copiedID}') "><!--
			...
			<div class="center" style="position:absolute; right: 7%; transform:translateY(-${demonList ? 19.5 : 16.25}vh); height: 10%">
				<a title="View level" onclick=levelRedirect('${x.id}') "><img style="margin-bottom: 4.5%" class="valign gdButton" src="../assets/view.png" height="105%"></a>
