...
<style>
	...
	.animation-in-right {
		animation: animationInRight 0.6s ease-out forwards;
	}

	.animation-out-left {
		animation: animationOutLeft 0.3s ease-in forwards;
	}

	.animation-in-left {
		animation: animationInLeft 0.6s ease-out forwards;
	}

	.animation-out-right {
		animation: animationOutRight 0.3s ease-in forwards;
	}
...
fetch('../api/gauntlets.php').then(res => res.json()).then(gauntlets => {
	...
	if(pages > 0) {
		$("#pageUp").show();
		$("#gauntletPage-1").addClass("animation-in-right").addClass("interactElement").attr('style', 'display: flex;');
	}
	$('#loading').hide()
	loading = false;
...
$('#pageUp').click(function() {
	page += 1;
	$(`#gauntletPage-${page}`).addClass("animation-in-right").removeClass("animation-out-left animation-in-left animation-out-right");
	$(`#gauntletPage-${page-1}`).addClass("animation-out-left").removeClass("animation-in-right animation-in-left animation-out-right");
	checkPage(false);
});

$('#pageDown').click(function() {
	page -= 1;
	$(`#gauntletPage-${page}`).addClass("animation-in-left").removeClass("animation-out-right animation-in-right animation-out-left");
	$(`#gauntletPage-${page+1}`).addClass("animation-out-right").removeClass("animation-in-left animation-in-right animation-out-left");
	checkPage(true);
});
...
function checkPage(toggle) {
	...
	$(`#gauntletPage-${page}`).addClass("interactElement").attr('style', 'display: flex');
	

	if(toggle){ 
		$(`#gauntletPage-${page+1}`).toggleClass("interactElement"); 
	}
	else {
		$(`#gauntletPage-${page-1}`).toggleClass("interactElement");
	}
